...
function symbolMarkup(v){
 ...
 if(v==="BOOK") return '<span class="sym-book"><i>♦</i></span>';
...
async function animateReels(finalGrid,startGrid){
 ...
 /* V25:
    Alle drei Reihen einer Walze sind beim Landen fest miteinander gekoppelt.
    Reihe 2/3 können nicht mehr nachträglich auf eine andere Rasterposition springen.
    Entscheidend: Wir messen die ECHTE Höhe einer bestehenden Symbolzelle und
    verwenden exakt diese Pixelhöhe für jede Zelle im bewegten Strip. */
 const prepared=reels.map((reel,c)=>{
   const reelRect=reel.getBoundingClientRect();
   const oldCells=[...reel.children];
   let cellH=reelRect.height/3;

   // Falls normale Symbolzellen vorhanden sind: echte gerenderte Höhe benutzen.
   if(oldCells.length>=3){
     const r0=oldCells[0].getBoundingClientRect();
     const r1=oldCells[1].getBoundingClientRect();
     const measured=Math.abs(r1.top-r0.top);
     if(measured>1) cellH=measured;
   }

   /* Startbild MUSS exakt das Bild vor dem Klick sein.
      Nicht finalGrid/grid verwenden, denn grid enthält hier bereits
      das neue Ergebnis. */
   const current=(startGrid && startGrid[c] && startGrid[c].length===3)
      ? startGrid[c].slice()
      : [...oldCells].slice(0,3).map(cell=>cell.dataset.sym).filter(Boolean);
   while(current.length<3) current.push(pick());

   const randomCells=[];
   for(let n=0;n<36;n++) randomCells.push(pick());

   // Die ersten 3 Zellen sind exakt das endgültige Ergebnis.
   // Die letzten 3 Zellen sind exakt das aktuelle sichtbare Ergebnis.
   const cells=[...finalGrid[c], ...randomCells, ...current];

   const strip=document.createElement("div");
   strip.className="v25-symbol-strip";

   cells.forEach((sym,i)=>{
     const cell=document.createElement("div");
     cell.className="v25-symbol-cell";
     cell.style.height=`${cellH}px`;
     cell.style.minHeight=`${cellH}px`;
     cell.style.maxHeight=`${cellH}px`;
     if(i<3) cell.classList.add("v25-final-cell");
     const symbol=makeSymbol(sym);
     cell.appendChild(symbol);
     strip.appendChild(cell);
   });

   // Exakt die letzten drei Zellen sichtbar machen.
   const startY=-(cells.length-3)*cellH;
   strip.style.transform=`translate3d(0,${startY}px,0)`;

   reel.classList.add("v25-reel-spinning");
   reel.replaceChildren(strip);

   return {reel,strip,c,cellH,startY,stop:stopTimes[c]};
 });

 void root.offsetWidth;
 const globalStart=performance.now();

 await Promise.all(prepared.map(({reel,strip,c,cellH,startY,stop})=>new Promise(resolve=>{
   const brakeDuration=340;
   const brakeStart=stop-brakeDuration;
   const totalDistance=-startY;

   // Konstanter Lauf; nur die letzten 340 ms bremsen.
   // Endpunkt ist mathematisch EXAKT y=0.
   const speed=totalDistance/(brakeStart + brakeDuration*0.75);
   const cruiseDistance=speed*brakeStart;
   const remaining=totalDistance-cruiseDistance;

   function tick(now){
     const elapsed=Math.min(stop,now-globalStart);
     let y;

     if(elapsed<=brakeStart){
       y=startY + speed*elapsed;
     }else{
       const q=Math.min(1,(elapsed-brakeStart)/brakeDuration);
       const eased=1-Math.pow(1-q,3);
       y=startY + cruiseDistance + remaining*eased;
     }
     ...
     if(elapsed<stop){
       ...
       // Alle drei finalen Zellen landen im selben Frame gemeinsam.
       strip.style.transform="translate3d(0,0,0)";
       reel.classList.remove("v25-reel-spinning");
       reel.classList.add("v25-reel-stopped");
       resolve();
     }
   }
   requestAnimationFrame(tick);
 ...
 // Kein DOM-Wechsel nach dem Stopp.
}
...
function featureGlyph(sym){
 const map={VALORIAN:"VALORIAN",DERION:"DERION",FACE209:"FACE 209",FACE193:"FACE 193",FACE96:"FACE 96",WARRIOR:"WARRIOR",PRIEST:"PRIEST",MAGE:"MAGE",ARCHER:"ARCHER",ASSASSIN:"ASSASSIN"};
 return map[sym]||sym||"✦";
}
...
async function showFreeIntro(count,sym){
 ...
 const candidates=["VALORIAN","DERION","FACE209","FACE193","FACE96","WARRIOR","PRIEST","MAGE","ARCHER","ASSASSIN"];
 for(let i=0;i<candidates.length;i++){
   setFeatureChosen(chosen,candidates[i]);
...
async function animateExpandSymbol(sym,featureIndex=0){
 ...
 for(const c of reelsContaining(sym)){
   ...
   for(const row of order){
     ...
     cell.classList.remove("v27-expand-change","v27-expand-in");
     cell.classList.add("v43-crossfade-cell");

     /* Bestehendes Symbol bleibt als echte Ausgangsebene erhalten. */
     const outgoing=document.createElement("div");
     outgoing.className="v43-expand-outgoing";
     while(cell.firstChild) outgoing.appendChild(cell.firstChild);

     const incoming=document.createElement("div");
     incoming.className="v43-expand-incoming";
     incoming.innerHTML=symbolMarkup(sym);

     cell.append(outgoing,incoming);

     /* Erst Ausgangsbild zeichnen, dann beide Ebenen gleichzeitig überblenden. */
     await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
     outgoing.classList.add("v43-hide");
     incoming.classList.add("v43-show");

     await sleep(115);

     /* Endzustand exakt dieselbe Symbolstruktur wie jedes normale Walzenfeld. */
     cell.replaceChildren();
     cell.innerHTML=symbolMarkup(sym);
     cell.dataset.sym=sym;
     cell.classList.remove("v43-crossfade-cell");
   }
   grid[c]=[sym,sym,sym];
...
async function showRetriggerFeature(count,newSym){
 ...
 setFeatureChosen(chosen,newSym);
...
async function spin(){
 ...
 /* ERST DANACH expandieren. Dabei ändern sich nur Symbolbilder,
    niemals Position/Höhe/Transform der Walzen. */
