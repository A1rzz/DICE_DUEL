...
function updateChrome(){
  ...
  if($("sideMode")) {
    const onStart=$("startMenu")&&!$("startMenu").classList.contains("hidden");
    const onSlots=$("slot")&&!$("slot").classList.contains("hidden");
    const activeMode=document.querySelector("#gameNavigation .mode.active");
    const visibleMode=activeMode?Number(activeMode.dataset.mode):(game?.max_players||selectedMode);
    $("sideMode").textContent=onStart?"Start":onSlots?"4Slots":visibleMode===10?"10 Spieler":"1vs1";
  }
  if($("sideStake")){
...
async function playFreeBook(count,sym,retrigger=false){
 featureRunning=true;
 /* Die untere Symbolanzeige darf erst nach der vollständigen Buchanimation
    erscheinen. Sie wird hier sofort ausgeblendet und erst vom anschließenden
    ui()-Aufruf wieder aktiviert. Das gilt auch bei einem Retrigger. */
 S("slotFreePanel")?.classList.add("hidden");
 S("slotStatusHud")?.classList.remove("feature-active");
...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 const startMenu=S("startMenu");
 const openStartMode=mode=>{
   startMenu?.classList.add("hidden");
   if(mode==="slot"){
     document.querySelectorAll(".view").forEach(x=>x.classList.add("hidden"));
     S("slot")?.classList.remove("hidden");
     document.querySelectorAll(".mode,.slot-mode-btn").forEach(x=>x.classList.remove("active"));
     S("slotModeBtn")?.classList.add("active");
   }else{
     selectedMode=Number(mode);
     document.querySelectorAll(".mode").forEach(btn=>btn.classList.toggle("active",Number(btn.dataset.mode)===selectedMode));
     S("slotModeBtn")?.classList.remove("active");
     show("home");
     syncLobbyModeLabel();
   }
   updateChrome();
 };
 document.querySelectorAll("[data-start-mode]").forEach(btn=>btn.addEventListener("click",()=>openStartMode(btn.dataset.startMode)));
 S("startHomeBtn")?.addEventListener("click",()=>{startMenu?.classList.remove("hidden");updateChrome()});
