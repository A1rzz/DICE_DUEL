...
async function animateReels(finalGrid){
 ...
 const randomTriplet=()=>[pick(),pick(),pick()];
 const stopTimes=[1850,2075,2325,2600,2900];
 const cruiseSpeed=2.55; // px/ms: konstant, KEIN Beschleunigen

 /* V21:
    - Das vorhandene Bild bleibt beim Start sichtbar.
    - Neue Symbole werden oberhalb des sichtbaren Bildes aufgebaut.
    - Der Strip startet exakt auf dem bisherigen Bild, kein schwarzer Start.
    - Alle Walzen beginnen gleichzeitig und sofort mit konstanter Geschwindigkeit.
    - KEINE Beschleunigungsphase.
    - Nur die letzten ~420 ms bremsen weich ab.
    - Finalposition wird mathematisch auf ein volles 3er-Fenster gelegt,
      deshalb kein letzter Snap/Ruckler. */
 const prepared=reels.map((reel,c)=>{
   const h=Math.max(1,reel.getBoundingClientRect().height);

   // Aktuell sichtbare drei Symbole aus dem bestehenden Grid übernehmen.
   const current = (grid && grid[c] && grid[c].length===3)
       ? grid[c].slice()
       : randomTriplet();

   const before=[];
   for(let n=0;n<16;n++) before.push(randomTriplet());

   // Reihenfolge im DOM: Zielbild oben, Zufallsbilder, aktuelles Bild unten.
   // Startposition zeigt exakt das aktuelle Bild. Bewegung Richtung +Y
   // lässt die darüber liegenden Bilder von OBEN in das Fenster laufen.
   const frames=[finalGrid[c].slice(), ...before, current];

   const strip=document.createElement("div");
   strip.className="v21-reel-strip";
   frames.forEach(group=>{
     const frame=document.createElement("div");
     frame.className="v21-reel-frame";
     frame.style.height=h+"px";
     group.forEach(sym=>frame.appendChild(makeSymbol(sym)));
     strip.appendChild(frame);
   });

   const startIndex=frames.length-1;
   const startY=-startIndex*h;
   reel.classList.add("v21-reel-spinning");
   reel.replaceChildren(strip);
   strip.style.transform=`translate3d(0,${startY}px,0)`;

   return {reel,strip,c,h,startY,stop:stopTimes[c]};
 });

 // Alle Walzen im exakt selben Renderframe starten.
 void root.offsetWidth;
 const globalStart=performance.now();

 await Promise.all(prepared.map(({reel,strip,c,h,startY,stop})=>new Promise(resolve=>{
   const brakeDuration=420;
   const brakeStart=stop-brakeDuration;

   // Bis zur Bremsphase konstante Geschwindigkeit.
   const cruiseDistance=cruiseSpeed*brakeStart;

   // Endpunkt muss exakt auf einem Frame liegen. Wir wählen den nächsten
   // passenden Frame oberhalb, sodass die Bewegung weiterhin nach unten läuft.
   const rawEnd=startY+cruiseDistance+(cruiseSpeed*brakeDuration*0.48);
   const endY=Math.min(0, Math.ceil(rawEnd/h)*h);

   // Tatsächliche Bremsdistanz.
   const brakeDistance=endY-(startY+cruiseDistance);
   ...
   function tick(now){
     ...
     if(elapsed<=brakeStart){
       // Sofort konstante Geschwindigkeit, keine Beschleunigung.
       y=startY + cruiseSpeed*elapsed;
     }else{
       const q=Math.min(1,(elapsed-brakeStart)/brakeDuration);
       // Smoothstep: Geschwindigkeit fällt kontinuierlich bis 0,
       // ohne Overshoot/Bounce.
       const eased=1-Math.pow(1-q,3);
       y=startY + cruiseDistance + brakeDistance*eased;
     }
     ...
     if(elapsed<stop){
       ...
       // Das sichtbare Fenster steht jetzt exakt auf einer Frame-Grenze.
       // Finalbild ohne schwarzen Zwischenframe übernehmen.
       requestAnimationFrame(()=>{
         reel.replaceChildren(...finalGrid[c].map(sym=>makeSymbol(sym)));
         reel.classList.remove("v21-reel-spinning");
         resolve();
       });
     }
   }
