...
async function animateReels(finalGrid){
 ...
 const prepared=reels.map((reel,c)=>{
   ...
   const current=(grid && grid[c] && grid[c].length===3)
      ? grid[c].slice()
      : [pick(),pick(),pick()];
...
async function animateExpandSymbol(sym,featureIndex=0){
 ...
 /* V26:
    Expansion ist vollständig von der Walzen-DOM getrennt.
    Das stehende Spinbild wird NICHT ersetzt, verschoben oder neu aufgebaut.
    Wir legen nur temporäre Symbol-Overlays über die drei sichtbaren Felder. */
 for(const c of cols){
   const reel=root.children[c];
   if(!reel)continue;

   const reelRect=reel.getBoundingClientRect();
   const cellH=reelRect.height/3;

   // Quellzeile aus dem echten Grid bestimmen.
   let sourceRow=grid[c].findIndex(v=>v===sym);
   if(sourceRow<0)continue;

   // Reihenfolge immer von der Fundstelle aus.
   let order;
   if(sourceRow===0) order=[0,1,2];
   else if(sourceRow===2) order=[2,1,0];
   else order=[1,0,2];

   const layer=document.createElement("div");
   layer.className="v26-expand-layer";
   layer.style.setProperty("--feature-index",String(featureIndex));
   reel.appendChild(layer);

   // Quelle zuerst sichtbar hervorheben.
   const source=document.createElement("div");
   source.className="v26-expand-symbol v26-expand-source";
   source.style.top=`${sourceRow*cellH}px`;
   source.style.height=`${cellH}px`;
   source.appendChild(makeSymbol(sym));
   layer.appendChild(source);

   await sleep(380);

   // Danach die anderen Felder nacheinander füllen.
   for(const row of order){
     if(row===sourceRow)continue;

     const target=document.createElement("div");
     target.className="v26-expand-symbol v26-expand-enter";
     target.style.top=`${row*cellH}px`;
     target.style.height=`${cellH}px`;
     target.appendChild(makeSymbol(sym));
     layer.appendChild(target);

     // Animation wirklich fertig werden lassen, bevor das nächste Feld kommt.
     await new Promise(resolve=>{
       let done=false;
   ...
   // Komplette Walze kurz hervorheben.
   layer.classList.add("v26-expand-complete");
   await sleep(420);

   // Erst JETZT intern Grid setzen. Das sichtbare Reel selbst bleibt unangetastet.
   grid[c]=[sym,sym,sym];

   // Overlay bleibt als finales expandiertes Bild bestehen.
   layer.classList.remove("v26-expand-complete");
   layer.classList.add("v26-expand-locked");

   // Kleine Pause vor der nächsten betroffenen Walze.
   await sleep(260);
 }
}
