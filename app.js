...
async function animateReels(finalGrid){
 ...
 const stopTimes=[1900,2150,2425,2725,3050];

 /* V20:
    Kein CSS-Keyframe-Trick mehr. Die Walze wird mit requestAnimationFrame
    pixelweise bewegt. y wird IMMER größer -> Bewegung auf dem Bildschirm
    garantiert VON OBEN NACH UNTEN.

    Alle Walzen starten gleichzeitig mit derselben px/s-Geschwindigkeit.
    Erst in der individuellen Bremsphase werden sie langsamer. */
 const prepared=reels.map((reel,c)=>{
   const h=Math.max(1,reel.getBoundingClientRect().height);
   const frames=[finalGrid[c].slice()];
   for(let n=0;n<18;n++) frames.push(randomTriplet());

   const strip=document.createElement("div");
   strip.className="v20-reel-strip";
   frames.forEach(group=>{
     const frame=document.createElement("div");
     frame.className="v20-reel-frame";
     frame.style.height=h+"px";
     group.forEach(sym=>frame.appendChild(makeSymbol(sym)));
     strip.appendChild(frame);
   });

   reel.classList.add("v20-reel-spinning");
   reel.replaceChildren(strip);
   return {reel,strip,c,h,stop:stopTimes[c],finalGrid};
 });

 const start=performance.now();
 const speed=7.4; // px/ms, identisch für alle Walzen am Anfang

 await Promise.all(prepared.map(({reel,strip,c,h,stop})=>new Promise(resolve=>{
   // Wir starten mehrere komplette Fensterhöhen oberhalb der Endposition.
   // Dadurch kommen neue Symbole sichtbar über der oberen Kante herein.
   const distance=Math.max(h*8, speed*stop*0.58);
   const startY=-distance;
   const brakeStart=0.56; // bis hier identische lineare Anfangsgeschwindigkeit

   function frame(now){
     const elapsed=Math.min(stop,now-start);
     const p=elapsed/stop;
     let y;

     if(p<=brakeStart){
       // gemeinsame lineare Startphase: gleiche Richtung + gleiche Geschwindigkeit
       const linearDistance=distance*brakeStart;
       y=startY + linearDistance*(p/brakeStart);
     }else{
       // weiche Bremsphase ohne Bounce/Overshoot
       const q=(p-brakeStart)/(1-brakeStart);
       const eased=1-Math.pow(1-q,3);
       y=startY + distance*brakeStart + distance*(1-brakeStart)*eased;
     }

     // y wächst von negativ Richtung 0 = Inhalt bewegt sich NACH UNTEN.
     ...
     if(elapsed<stop){
       ...
       // Das Finalbild ist bereits die oberste Frame-Gruppe des Strips.
       // Zwei Frames später wird unsichtbar auf die normalen Zellen umgestellt.
       requestAnimationFrame(()=>requestAnimationFrame(()=>{
         reel.replaceChildren(...finalGrid[c].map(sym=>makeSymbol(sym)));
         reel.classList.remove("v20-reel-spinning");
         resolve();
       }));
     }
   }
