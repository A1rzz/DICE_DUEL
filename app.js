...
async function animateExpandSymbol(sym,featureIndex=0){
 ...
 for(const c of reelsContaining(sym)){
   ...
   for(const row of order){
     ...
     cell.classList.add("v42-crossfade-cell");

     const incoming=document.createElement("div");
     incoming.className="v42-expand-incoming";
     incoming.appendChild(makeSymbol(sym));
     cell.appendChild(incoming);

     /* Zwei Frames garantieren, dass der Browser den Startzustand zeichnet,
        bevor die Transition beginnt. */
     await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
     incoming.classList.add("v42-show");

     await sleep(150);

     /* Erst NACH dem sichtbaren Übergang wird das alte Symbol entfernt. */
     cell.replaceChildren(makeSymbol(sym));
     cell.classList.remove("v42-crossfade-cell");
   }
   grid[c]=[sym,sym,sym];
...
function lockVisibleGrid(){
 ...
 for(let c=0;c<5;c++){
   ...
   reel.className="bod-reel v27-fixed-reel";
 }
}
