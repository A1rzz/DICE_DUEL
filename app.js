...
async function animateExpandSymbol(sym,featureIndex=0){
 ...
 /* Jede Walze einzeln. Innerhalb der Walze beginnt die Expansion genau
    an der Position, an der das Feature-Symbol gelandet ist, und läuft
    von dort Feld für Feld weiter. Kein harter Komplett-Austausch. */
 for(const c of cols){
   ...
   const order=[0,1,2].sort((a,b)=>Math.abs(a-source)-Math.abs(b-source) || a-b);

   reel.classList.add("reel-expanding");
   const sourceEl=reel.children[source];
   sourceEl?.classList.add("expand-origin");
   await sleep(320);
   ...
   for(const r of order){
     ...
     if(grid[c][r]===sym){
       cell.classList.add("expand-lock");
       await sleep(180);
       continue;
     }

     cell.classList.add("expand-cell-out");
     await sleep(170);

     const fresh=makeSymbol(sym);
     fresh.classList.add("expand-cell-in");
     fresh.style.setProperty("--expand-from", String(source-r));
     reel.replaceChild(fresh,cell);
     grid[c][r]=sym;

     /* Deutlich sichtbare Pause zwischen jedem einzelnen Feld. */
     await sleep(310);
     fresh.classList.remove("expand-cell-in");
     fresh.classList.add("expand-lock");
   }

   grid[c]=[sym,sym,sym];
   await sleep(360);
   [...reel.children].forEach(el=>el.classList.remove("expand-origin","expand-lock"));
