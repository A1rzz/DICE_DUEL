...
async function animateReels(finalGrid){
 ...
 const stopTimes=[1800,2025,2275,2550,2850];

 /* V23
    Der bewegte Strip bleibt auch NACH dem Stopp im DOM.
    Es gibt keinerlei replaceChildren() am Ende des Spins.
    Damit kann nach dem sichtbaren Stillstand kein Symbol mehr wechseln.

    Die Bewegung:
    - startet aus dem aktuell sichtbaren Bild
    - kommt von oben nach unten
    - startet bei allen Walzen gleichzeitig
    - keine Beschleunigung
    - kurze, weiche Bremsphase
    - finalGrid ist die tatsächliche Landeposition des Strips */
 ...
 const prepared=reels.map((reel,c)=>{
   ...
   const middle=[];
   for(let n=0;n<18;n++) middle.push(randomTriplet());

   // finalGrid liegt an Position 0; current ganz unten.
   const frames=[finalGrid[c].slice(), ...middle, current];

   const strip=document.createElement("div");
   strip.className="v23-reel-strip";

   frames.forEach((group,i)=>{
     const frame=document.createElement("div");
     frame.className="v23-reel-frame";
     if(i===0) frame.classList.add("v23-final-frame");
     frame.style.height=h+"px";
     group.forEach(sym=>frame.appendChild(makeSymbol(sym)));
     strip.appendChild(frame);
   });

   const startY=-(frames.length-1)*h;
   strip.style.transform=`translate3d(0,${startY}px,0)`;
   reel.classList.add("v23-reel-spinning");
   reel.replaceChildren(strip);

   return {reel,strip,c,h,startY,stop:stopTimes[c]};
 });

 // Ein gemeinsamer Startframe.
 void root.offsetWidth;
 const globalStart=performance.now();

 await Promise.all(prepared.map(({reel,strip,c,h,startY,stop})=>new Promise(resolve=>{
   const brakeDuration=330;       // kürzer als V22 -> kein langes Kriechen
   const brakeStart=stop-brakeDuration;
   const totalDistance=-startY;

   // Cubic ease-out hat über die Bremsphase eine mittlere Fortschrittsrate
   // von 0.75. Geschwindigkeit so bestimmen, dass exakt y=0 erreicht wird.
   const brakeAverageFactor=0.75;
   const speed=totalDistance/(brakeStart + brakeDuration*brakeAverageFactor);
   ...
   function tick(now){
     ...
     if(elapsed<=brakeStart){
       // Sofort konstante Geschwindigkeit, keine Beschleunigungsphase.
       y=startY + speed*elapsed;
     }else{
       const q=Math.min(1,(elapsed-brakeStart)/brakeDuration);

       // Smoothstep: Geschwindigkeit geht weich gegen 0,
       // ohne Bounce und ohne langes Nachkriechen.
     ...
     if(elapsed<stop){
       ...
       // EXAKT finalGrid. Danach keinerlei DOM-Austausch mehr.
       strip.style.transform="translate3d(0,0,0)";
       reel.classList.remove("v23-reel-spinning");
       reel.classList.add("v23-reel-stopped");
       resolve();
     }
   }
 ...
 // WICHTIG V23:
 // Kein replaceChildren(...finalGrid...) hier.
 // Die finalen Frames des Strips SIND das endgültige sichtbare Spielfeld.
}
