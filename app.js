...
async function animateReels(finalGrid){
 ...
 const cruiseSpeed=2.55;

 /* V22:
    Der sichtbare Landepunkt IST bereits finalGrid.
    Es gibt nach dem Stillstand keinen Austausch der Symbole mehr. */
 const prepared=reels.map((reel,c)=>{
   ...
   // Strip: Zielbild ganz oben, Zwischenbilder, aktuelles Bild ganz unten.
   // Start zeigt exakt das aktuelle Bild.
   const middle=[];
 ...
 await Promise.all(prepared.map(({reel,strip,c,h,startY,stop})=>new Promise(resolve=>{
   const brakeDuration=460;
   const brakeStart=stop-brakeDuration;

   // Wir berechnen die konstante Geschwindigkeit so, dass die Walze
   // exakt bei y=0 landet. y=0 zeigt frames[0] = finalGrid.
   const totalDistance=-startY;
   const brakeFactor=0.48;
   const speed=totalDistance/(brakeStart + brakeDuration*brakeFactor);
   const cruiseDistance=speed*brakeStart;
   const brakeDistance=totalDistance-cruiseDistance;
   ...
   function tick(now){
     ...
     if(elapsed<=brakeStart){
       ...
       // weiches Ausrollen bis exakt y=0, ohne Overshoot
     ...
     if(elapsed<stop){
       ...
       // Wichtig: Das sichtbare Bild ist jetzt bereits finalGrid.
       strip.style.transform="translate3d(0,0,0)";

       // Kein replaceChildren(finalGrid) mehr direkt beim Stop.
       // Der Strip bleibt stehen; dadurch kann optisch nichts wechseln.
       reel.classList.remove("v21-reel-spinning");
       reel.classList.add("v22-reel-landed");
       resolve();
     }
   }
   requestAnimationFrame(tick);
 ...
 // Erst nachdem ALLE Walzen fertig sind, normalisieren wir den DOM.
 // Dabei bleibt exakt dasselbe finalGrid sichtbar; kein Zwischenframe.
 await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));
 reels.forEach((reel,c)=>{
   reel.replaceChildren(...finalGrid[c].map(sym=>makeSymbol(sym)));
   reel.classList.remove("v22-reel-landed");
 });
}
