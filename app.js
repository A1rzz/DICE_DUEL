...
async function animateReels(finalGrid){
 ...
 /* V18:
    - ALLE 5 Walzen starten exakt gleichzeitig.
    - ALLE starten mit derselben Geschwindigkeit.
    - Erst beim Abbremsen unterscheiden sie sich.
    - Jede Walze stoppt als kompletter 3er-Block; keine Reihe rutscht nach. */
 const stopTimes=[1750,1950,2180,2440,2730];

 const runs=reels.map((reel,c)=>{
   reel.classList.add("v18-reel-spinning");

   const groups=[];
   // Genug Bilder für eine gemeinsame schnelle Startphase.
   for(let n=0;n<12+c*2;n++) groups.push(randomTriplet());
   groups.push(finalGrid[c].slice());

   const strip=document.createElement("div");
   strip.className="v18-reel-strip";

   groups.forEach(group=>{
     const frame=document.createElement("div");
     frame.className="v18-reel-frame";
     group.forEach(sym=>frame.appendChild(makeSymbol(sym)));
     strip.appendChild(frame);
   });

   reel.replaceChildren(strip);
   strip.style.setProperty("--v18-steps",String(groups.length-1));
   strip.style.setProperty("--v18-stop",`${stopTimes[c]}ms`);
   return {reel,strip,c,stop:stopTimes[c]};
 });

 // Ein Reflow für alle, danach derselbe Startzeitpunkt.
 void root.offsetWidth;
 runs.forEach(({strip})=>strip.classList.add("v18-run"));

 await Promise.all(runs.map(async ({reel,c,stop})=>{
   await sleep(stop);

   // Finales Bild einmalig einsetzen: alle drei Reihen gleichzeitig.
   reel.replaceChildren(...finalGrid[c].map(sym=>makeSymbol(sym)));
   reel.classList.remove("v18-reel-spinning");
   reel.classList.add("v18-reel-stopped");
   await sleep(120);
   reel.classList.remove("v18-reel-stopped");
 }));
}
