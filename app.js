...
function ui(msg){
 ...
 S("slotActiveLines").textContent=lineCount;S("slotFreeSpins").textContent=free;S("slotExpandSymbol").textContent=expandSymbols.length?expandSymbols.map(featureGlyph).join(" + "):(expand?featureGlyph(expand):"—");renderFeatureStack();
...
function renderFeatureStack(){
 ...
 box.innerHTML=`<span>EXPANDIEREND</span>${expandSymbols.map((s,i)=>`<b>${i+1}. ${featureGlyph(s)}</b>`).join("")}`;
}
