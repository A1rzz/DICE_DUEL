...
function v47PrepareReelsForExpansion(){
 ...
 for(let c=0;c<5;c++){
   ...
   const stripCells=[...strip.children].slice(0,3);
...
async function animateExpandSymbol(sym,featureIndex=0){
 ...
 for(const c of reelsContaining(sym)){
   ...
   for(const row of order){
     ...
     /* Nur kurze Pause zwischen den einzelnen Feldern – das Symbol selbst
        springt sofort um und bleibt durchgehend sichtbar. */
     await sleep(75);
   }
   grid[c]=[sym,sym,sym];
