...
const activeLines=()=>Math.max(1,Math.min(10,bet));
...
async function animateReels(finalGrid){
 ...
 await Promise.all(reels.map(({reel,strip,lead},c)=>new Promise(resolve=>{
   ...
   setTimeout(()=>{
     ...
     /* Wichtig: Nach dem Stopp die lange Animationsliste nicht verschoben stehen lassen. */
...
async function spin(){
 ...
 draw(); /* endgültige 3x5 Symbole vollständig anzeigen */
