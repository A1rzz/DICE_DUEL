...
function ui(msg){
 ...
 S("slotActiveLines").textContent=lineCount;S("slotFreeSpins").textContent=free;S("slotExpandSymbol").textContent=expandSymbols.length?expandSymbols.join(" + "):(expand||"—");
...
async function showFreeOutro(total){
 ...
 S("freeOverlayText").textContent=`GEWINN AUS DER KOMPLETTEN FREISPIEL-RUNDE`;
 S("featureChosenSymbol").textContent="♦";
 S("featureSymbolLabel").textContent=expandSymbols.length?`EXPANDIEREND: ${expandSymbols.join(" + ")}`:"BOOK OF DICE";
 S("freeOverlayMoney").innerHTML=`<div class="free-total-label">GESAMT</div><div class="free-total-value">${moneyHTML(total)}</div>`;
 o.className="free-overlay free-in free-final v14-total";
 await sleep(4200);
 o.classList.add("free-out");
 await sleep(650);
 o.className="free-overlay hidden";
...
async function animateExpandSymbol(sym){
 const root=S("slotReels");
 if(!root)return;
 ...
 for(const c of cols){
   ...
   if(!reel)continue;
   const originalRows=[];
   for(let r=0;r<3;r++)if(grid[c][r]===sym)originalRows.push(r);
   originalRows.forEach(r=>reel.children[r]?.classList.add("expand-source"));
   await sleep(260);
   reel.classList.add("reel-expanding");
   for(let r=0;r<3;r++){
     if(grid[c][r]!==sym){
       const old=reel.children[r];
       if(old){
         old.classList.add("expand-out");
         await sleep(115);
         const fresh=makeSymbol(sym);
         fresh.classList.add("expand-in");
         reel.replaceChild(fresh,old);
       }
     }else{
       reel.children[r]?.classList.add("expand-hold");
     }
   }
   grid[c]=[sym,sym,sym];
   await sleep(330);
   [...reel.children].forEach(el=>el.classList.remove("expand-source","expand-in","expand-hold"));
   reel.classList.remove("reel-expanding");
 }
}
async function expandAllActiveSymbols(){
 ...
 for(let i=0;i<expandSymbols.length;i++){
   ...
   if(reelsContaining(sym).length){
     ui(`EXPANDIEREND ${i+1}/${expandSymbols.length}: ${sym}`);
     await animateExpandSymbol(sym);
     await sleep(260);
   }
 }
}
async function showRetriggerFeature(count,newSym){
 /* Dasselbe Buch wird bei erneutem Feature komplett abgespielt.
    Danach bleibt das alte Symbol aktiv und das neue kommt als zweites dazu. */
 await showFreeIntro(count,newSym);
}
async function spin(){
