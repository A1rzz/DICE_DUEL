...
let balance=1000, bet=10, grid=[], busy=false, free=0, expand=null, auto=false;
const activeLines=()=>Math.max(1,Math.min(10,Math.floor(bet/10)));
...
function draw(winCells=new Set()){
 const root=S("slotReels"); if(!root)return; root.innerHTML="";
 for(let c=0;c<5;c++){
  const reel=document.createElement("div"), strip=document.createElement("div");
  reel.className="reel"; strip.className="reel-strip";
  (grid[c]||[pick(),pick(),pick()]).forEach((v,r)=>{const d=makeSymbol(v);if(winCells.has(c+"-"+r))d.classList.add("win");strip.appendChild(d)});
  reel.appendChild(strip); root.appendChild(reel);
 }
}
...
function ui(msg){
 ...
 S("slotActiveLines").textContent=activeLines();S("slotFreeSpins").textContent=free;S("slotExpandSymbol").textContent=expand||"—";
...
function evaluate(){
 ...
 for(let li=0;li<activeLines();li++){
  ...
  if(target!=="BOOK"&&count>=3){
   total+=(pays[target]?.[count-1]||0)*(bet/10);
...
async function animateReels(finalGrid){
 const root=S("slotReels");root.innerHTML="";const h=cellHeight(), reels=[];
 for(let c=0;c<5;c++){
  const reel=document.createElement("div"),strip=document.createElement("div");
  reel.className="reel spinning";strip.className="reel-strip";
  const lead=Array.from({length:22+c*3},pick), vals=[...lead,...finalGrid[c]];
  vals.forEach(v=>strip.appendChild(makeSymbol(v)));reel.appendChild(strip);root.appendChild(reel);reels.push({reel,strip,lead:lead.length});strip.style.transform="translateY(0px)";
 }
 await sleep(80);
 await Promise.all(reels.map(({reel,strip,lead},c)=>new Promise(resolve=>{
   const ms=2400+c*430; /* deliberately slower, readable reels */
   strip.style.transition=`transform ${ms}ms cubic-bezier(.08,.58,.12,1)`;
   requestAnimationFrame(()=>strip.style.transform=`translateY(${-lead*h}px)`);
   setTimeout(()=>{reel.classList.remove("spinning");resolve()},ms+80);
 })));
}
async function spin(){
 ...
 grid=Array.from({length:5},()=>Array.from({length:3},pick));await animateReels(grid);
...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 S("slotBetDown")?.addEventListener("click",()=>{if(!busy){bet=Math.max(10,bet-10);ui()}});
 S("slotBetUp")?.addEventListener("click",()=>{if(!busy){bet=Math.min(100,bet+10);ui()}});
 S("slotMaxBet")?.addEventListener("click",()=>{if(!busy){bet=100;ui()}});
