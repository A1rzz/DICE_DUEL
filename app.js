...
async function animateReels(finalGrid,startGrid){
 ...
 const prepared=reels.map((reel,c)=>{
   ...
   for(let n=0;n<30;n++)randomSequence.push(pick());
 ...
 await Promise.all(prepared.map(({reel,track,c,stop,distance})=>new Promise(resolve=>{
   const brakeDuration=600;
   const brakeStart=stop-brakeDuration;
   /* Match the velocity at the hand-off from constant travel to cubic
      braking. This removes the small acceleration jerk that was visible
      immediately before each reel slowed down. */
   const cruiseProgress=(3*brakeStart)/(brakeDuration+3*brakeStart);
   function tick(now){
     const elapsed=Math.min(stop,now-globalStart);
     let progress;
     if(elapsed<=brakeStart){
       progress=(elapsed/brakeStart)*cruiseProgress;
     }else{
       const q=Math.min(1,(elapsed-brakeStart)/brakeDuration);
       progress=cruiseProgress+(1-cruiseProgress)*(1-Math.pow(1-q,3));
     }
     track.style.transform=`translate3d(0,${-distance*(1-progress)}px,0)`;
     if(elapsed<stop){
