...
let balance=1000, bet=1, lineCount=10, grid=[], busy=false, free=0, expand=null, auto=false;
...
function evaluate(){
 ...
 for(let li=0;li<activeLines();li++){
  ...
  if(target!=="BOOK"&&count>=3){
   total+=(pays[target]?.[count-1]||0)*(bet/activeLines());
...
async function animateWin(result){
 ...
 await new Promise(resolve=>{
   const tick=now=>{
     ...
     S("slotWin").textContent=Math.floor(target*(1-Math.pow(1-p,3)));
...
async function spin(){
 ...
 draw();
 const w=evaluate();balance+=w.total;draw(w.cells);drawPaylines(w.wonLines);S("slotWin").textContent="0";
 ui(books>=3&&!fs?`8 Freispiele! Expandierendes Symbol: ${expand}`:w.total?`GEWINN auf Linie ${w.wonLines.map(x=>x+1).join(", ")}!`:"Kein Gewinn.");
...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 S("slotMaxBet")?.addEventListener("click",()=>{if(!busy){bet=10;lineCount=10;ui()}});
