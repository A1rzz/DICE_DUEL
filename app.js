...
async function animateReels(finalGrid,startGrid){
 ...
 const prepared=reels.map((reel,c)=>{
   ...
   const sequence=[];
   for(let n=0;n<30;n++)sequence.push(pick());
   sequence.push(...finalGrid[c]);
   ...
   return {reel,track,c,cellH,stop:stopTimes[c],distance:(sequence.length-3)*cellH};
 });
 ...
 await Promise.all(prepared.map(({reel,track,c,stop,distance})=>new Promise(resolve=>{
   ...
   function tick(now){
     ...
     if(elapsed<=brakeStart){
       progress=(elapsed/brakeStart)*0.82;
     }else{
       const q=Math.min(1,(elapsed-brakeStart)/brakeDuration);
       progress=0.82+0.18*(1-Math.pow(1-q,3));
     }
     track.style.transform=`translate3d(0,${-distance*progress}px,0)`;
     if(elapsed<stop){
       requestAnimationFrame(tick);
     }else{
       /* Commit the result underneath in one frame, then remove the overlay. */
       const finalCells=finalGrid[c].map(makeSymbol);
       reel.replaceChildren(...finalCells);
       reel.className="reel reel-static v27-fixed-reel classic-reel-stopped";
       resolve();
     }
   }
   requestAnimationFrame(tick);
...
async function animateExpandSymbol(sym,featureIndex=0){
 ...
 for(const c of reelsContaining(sym)){
   ...
   for(const row of order){
     ...
     /* cell is already the fixed .bod-symbol element. Nesting another
        .bod-symbol inside it keeps the old cell geometry/content rules alive
        and makes the expansion appear unchanged. Replace only the artwork
        and symbol id in the existing cell. */
