...
let balance=100000, lineBet=10, lineCount=1, grid=[], busy=false, free=0, expand=null, auto=false, freeSession=false, freeSessionTotal=0, freeSessionStart=0; /* Werte in Silber; 100 Silber = 1 Gold */
...
function ensureFreeOverlay(){
 ...
 if(o)return o;
 o=document.createElement("div");o.id="slotFreeOverlay";o.className="free-overlay hidden";
 ...
 <div class="feature-book-stage">
   ...
   <h2 id="freeOverlayTitle">DAS BUCH DER WÜRFEL</h2>
   <div class="feature-book">
     ...
     <div class="feature-page feature-right">
       ...
       <div class="feature-label">EXPANDIERENDES SYMBOL</div>
     </div>
     ...
     <div class="feature-flip f4"><div>✦</div><div>⚔</div></div>
   </div>
   <div class="feature-count"><b id="freeOverlayBig">8</b><span>FREISPIELE</span></div>
   <p id="freeOverlayText">DAS BUCH WÄHLT DEIN FEATURE …</p>
   <div id="freeOverlayMoney" class="free-money"></div>
 </div>`;
 document.querySelector("#slot .bod-cabinet")?.appendChild(o);return o;
}
function featureGlyph(sym){
 const map={A:"A",K:"K",Q:"Q",J:"J","10":"10",CROWN:"♛",SCARAB:"◆",PHARAOH:"♜",ANKH:"☥",VASE:"⚱",SWORDS:"⚔"};
 return map[sym]||"⚔";
}
async function showFreeIntro(count,sym){
 ...
 S("freeOverlayTitle").textContent="DAS BUCH DER WÜRFEL";
 S("freeOverlayBig").textContent=count;
 S("freeOverlayText").textContent="DAS BUCH WÄHLT DEIN FEATURE …";
 S("freeOverlayMoney").innerHTML="";
 chosen.textContent="✦";
 o.classList.remove("hidden","free-out","free-final","feature-chosen");
 void o.offsetWidth;
 o.classList.add("free-in","book-playing");
 const cycle=["♛","◆","⚱","A","K","Q","⚔"];
 for(let i=0;i<cycle.length;i++){ await sleep(470); chosen.textContent=cycle[i]; }
 await sleep(700);
 chosen.textContent=featureGlyph(sym);
 o.classList.add("feature-chosen");
 S("freeOverlayText").textContent=`EXPANDIERENDES SYMBOL: ${sym}`;
 await sleep(1900);
 o.classList.add("free-out");await sleep(600);
 o.classList.add("hidden");o.classList.remove("free-in","free-out","book-playing","feature-chosen");
}
async function showFreeOutro(total){
 const o=ensureFreeOverlay();
 S("freeOverlayKicker").textContent="FREISPIELE BEENDET";
 S("freeOverlayTitle").textContent="GESAMTGEWINN";
 S("freeOverlayBig").textContent="★";
 S("freeOverlayText").textContent="DEIN GEWINN AUS DEN FREISPIELEN";
 S("freeOverlayMoney").innerHTML=moneyHTML(total);
 o.classList.remove("hidden","free-out");o.classList.add("free-in","free-final");
 await sleep(3200);o.classList.add("free-out");await sleep(550);o.classList.add("hidden");o.classList.remove("free-in","free-out","free-final");
}
async function spin(){
 if(busy)return;
 const fs=free>0;
 ...
 busy=true;
 ["slotSpin","slotBetDown","slotBetUp","slotLineDown","slotLineUp","slotMaxBet"].forEach(id=>{if(S(id))S(id).disabled=true});
 /* Sobald gespielt wird, verschwinden die Vorschau-Linien. Nur echte Gewinnlinien erscheinen später. */
 drawPaylines([],false);
 if(fs)free--;else balance-=totalStake();
 S("slotWin").innerHTML=moneyHTML(0);ui("Die Walzen drehen …");
 ...
 if(books>=3&&!fs){
   free=8;
   expand=symbols.filter(x=>x!=="BOOK")[Math.floor(Math.random()*9)];
   freeSession=true;freeSessionTotal=0;freeSessionStart=balance;justTriggered=true;
 }
 ...
 const w=evaluate();
 balance+=w.total;
 if(fs&&freeSession)freeSessionTotal+=w.total;
 draw(w.cells);
 drawPaylines(w.wonLines,false);
 S("slotWin").innerHTML=moneyHTML(0);

 ui(justTriggered?`8 Freispiele! Expandierendes Symbol: ${expand}`:
    w.total?`GEWINN: ${moneyText(w.total)}${w.wonLines.length?` · Linie ${w.wonLines.map(x=>x+1).join(", ")}`:" · Scatter"}!`:
    "Kein Gewinn.");

 if(w.total)await animateWin(w);

 if(justTriggered) await showFreeIntro(8,expand);

 busy=false;
 ["slotSpin","slotBetDown","slotBetUp","slotLineDown","slotLineUp","slotMaxBet"].forEach(id=>{if(S(id))S(id).disabled=false});

 if(free>0){
   await sleep(1050);
   spin();
 }else if(fs&&freeSession){
   const total=freeSessionTotal;
   freeSession=false;
   await showFreeOutro(total);
   S("slotMessage").textContent=`FREISPIELE BEENDET · GESAMTGEWINN: ${moneyText(total)}`;
   expand=null;ui();
   if(auto){await sleep(1000);spin()}
 }else if(auto){
   await sleep(1000);spin();
 }
}
