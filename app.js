...
let balance=100000, lineBet=10, lineCount=1, grid=[], busy=false, free=0, expand=null, auto=false, freeSession=false, freeSessionTotal=0, freeSessionStart=0, featureRunning=false; /* Werte in Silber; 100 Silber = 1 Gold */
...
async function showFreeOutro(total){
 ...
 S("freeOverlayUnit").textContent="";
 S("freeOverlayText").textContent="DEIN GESAMTGEWINN AUS DEN FREISPIELEN";
 S("featureChosenSymbol").textContent="♦";
 S("featureSymbolLabel").textContent="BOOK OF DICE";
 S("freeOverlayMoney").innerHTML=moneyHTML(total);
 o.className="free-overlay free-in free-final";
 await sleep(3000);
 o.classList.add("free-out");
 await sleep(600);
 o.className="free-overlay hidden";
...
function chooseExpandSymbol(){
 const eligible=symbols.filter(x=>x!=="BOOK");
 return eligible[Math.floor(Math.random()*eligible.length)];
}
function expandReelsForFeature(){
 if(!expand)return new Set();
 const expanded=new Set();
 for(let c=0;c<5;c++){
   if(grid[c].includes(expand)){
     grid[c]=[expand,expand,expand];
     expanded.add(c);
   }
 }
 return expanded;
...
async function spin(){
 ...
 if(fs) free--;
 else balance-=totalStake();

 S("slotWin").innerHTML=moneyHTML(0);
 ui(fs?`FREISPIEL ${10-free}/10 · Die Walzen drehen …`:"Die Walzen drehen …");

 grid=Array.from({length:5},()=>Array.from({length:3},pick));
 await animateReels(grid);

 const books=grid.flat().filter(x=>x==="BOOK").length;
 let justTriggered=false, retriggered=false;

 /* Hauptfeature: Die Freispiele werden vorbereitet, aber NICHT gestartet,
    bevor die komplette Buch-Sequenz beendet ist. */
 if(books>=3&&!fs){
   free=10;
   expand=chooseExpandSymbol();
   freeSession=true;
   freeSessionTotal=0;
 ...
 /* Während laufender Freispiele erweitern Treffer des gewählten Symbols
    die komplette betreffende Walze. */
 if(fs&&expand) expandReelsForFeature();
 ...
 /* Bücher können in den Freispielen weitere 10 Freispiele geben.
    Das expandierende Symbol bleibt dabei unverändert. */
 if(fs&&books>=3){
   free+=10;
   retriggered=true;
 }
 ...
 if(justTriggered){
   ui(w.total?`BONUS! ${moneyText(w.total)} Scatter-Gewinn · Feature startet …`:"BONUS! Feature startet …");
 }else if(retriggered){
   ui(`+10 FREISPIELE! Noch ${free} Freispiele · Expandierend: ${expand}`);
 }else{
   ui(w.total?`GEWINN: ${moneyText(w.total)}${w.wonLines.length?` · Linie ${w.wonLines.map(x=>x+1).join(", ")}`:" · Scatter"}!`:
      fs?`Noch ${free} Freispiele · Expandierend: ${expand}`:"Kein Gewinn.");
 }

 if(w.total) await animateWin(w);

 if(justTriggered){
   /* Erst jetzt: Walzen stehen, Gewinn ist abgearbeitet, Spiel hält an.
      Dann Buch öffnen + Seiten rechts→links + Symbolwahl. */
   await showFreeIntro(10,expand);
   ui(`10 FREISPIELE · EXPANDIEREND: ${expand}`);
 }

 busy=false;
 setGameControlsLocked(false);

 /* Automatische Freispiel-Kette erst NACH der Feature-Animation. */
 if(free>0){
   await sleep(900);
   return spin();
 }
