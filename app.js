...
const moneyHTML=v=>{const m=moneyParts(v);return `<span class="coin-gold">●</span> ${m.gold} <span class="coin-silver">●</span> ${String(m.silver).padStart(2,"0")}`};
const moneyText=v=>{const m=moneyParts(v);return `${m.gold} Gold ${m.silver} Silber`};
...
async function spin(){
 ...
 if(fs)free--;else balance-=totalStake();S("slotWin").innerHTML=moneyHTML(0);drawPaylines([]);ui("Die Walzen drehen …");
