...
async function animateReels(finalGrid){
 ...
 /* V17:
    Jede der 5 Walzen bewegt sich als EIN zusammenhängender Strip.
    Die drei sichtbaren Felder einer Walze stoppen gleichzeitig.
    Nach dem Stop wird NICHT mehr an Reihe 2/3 nachanimiert. */
 ...
 for(let c=0;c<reels.length;c++){
   const reel=reels[c];
   reel.classList.add("v17-reel-spinning");

   // Ein gemeinsamer Strip enthält mehrere 3er-Gruppen + am Ende das Zielbild.
   const groups=[];
   for(let n=0;n<6;n++)groups.push(randomTriplet());
   groups.push(finalGrid[c].slice());

   const strip=document.createElement("div");
   strip.className="v17-reel-strip";

   groups.forEach(group=>{
     const frame=document.createElement("div");
     frame.className="v17-reel-frame";
   ...
   // Strip startet oben und fährt als komplette Einheit zum finalen 3er-Bild.
   strip.style.setProperty("--v17-steps", String(groups.length-1));
   void strip.offsetWidth;
   strip.classList.add("v17-run");

   // Walzen stoppen nacheinander, aber innerhalb einer Walze alle 3 Reihen zusammen.
   const duration=1050 + c*135;
   await sleep(duration);

   // Nach dem Stopp finalen Zustand EINMAL einsetzen. Keine Einzelanimation der Reihen.
   reel.replaceChildren(...finalGrid[c].map(sym=>makeSymbol(sym)));
   reel.classList.remove("v17-reel-spinning","v17-reel-stopped");
   reel.classList.add("v17-reel-stopped");
   await sleep(125);
   reel.classList.remove("v17-reel-stopped");
 }
}
