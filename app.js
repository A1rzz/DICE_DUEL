...
async function animateReels(finalGrid){
 ...
 /* V24 – echter kontinuierlicher Symbolstreifen.
    Nicht mehr 3er-Frames verschieben, sondern einzelne Symbolzellen.
    Dadurch bleibt das Raster beim Lauf und beim Stopp exakt erhalten. */
 const prepared=reels.map((reel,c)=>{
   const reelH=Math.max(1,reel.getBoundingClientRect().height);
   const cellH=reelH/3;
   ...
   // DOM von oben nach unten:
   // finalGrid ist ganz oben, danach Zufallssymbole, ganz unten das aktuelle Bild.
   // Beim Start ist exakt das aktuelle Bild sichtbar.
   const randomCells=[];
   for(let n=0;n<33;n++) randomCells.push(pick());
   const cells=[...finalGrid[c], ...randomCells, ...current];

   const strip=document.createElement("div");
   strip.className="v24-symbol-strip";

   cells.forEach((sym,i)=>{
     const cell=document.createElement("div");
     cell.className="v24-symbol-cell";
     cell.style.height=cellH+"px";
     if(i<3) cell.classList.add("v24-final-cell");
     cell.appendChild(makeSymbol(sym));
     strip.appendChild(cell);
   });

   // Die letzten drei Einzelzellen (= current) sind beim Start sichtbar.
   const startY=-(cells.length-3)*cellH;
   strip.style.transform=`translate3d(0,${startY}px,0)`;

   reel.classList.add("v24-reel-spinning");
   reel.replaceChildren(strip);

   return {reel,strip,c,reelH,cellH,startY,stop:stopTimes[c]};
 });
 ...
 await Promise.all(prepared.map(({reel,strip,c,cellH,startY,stop})=>new Promise(resolve=>{
   // Konstanter Lauf, nur kurze sanfte Bremsung.
   const brakeDuration=360;
   const brakeStart=stop-brakeDuration;
   const totalDistance=-startY;

   // easeOutCubic über die Bremsphase hat Mittelwert 0.75.
   // So wird exakt y=0 erreicht, ohne Snap am Ende.
   ...
   function tick(now){
     ...
     if(elapsed<stop){
       ...
       // Exakt auf den ersten drei Einzelzellen landen = finalGrid.
       strip.style.transform="translate3d(0,0,0)";
       reel.classList.remove("v24-reel-spinning");
       reel.classList.add("v24-reel-stopped");
       resolve();
     }
   }
   requestAnimationFrame(tick);
 ...
 // Kein DOM-Tausch nach dem Stopp.
 // Der Strip bleibt exakt auf finalGrid stehen.
}
