...
async function animateReels(finalGrid){
 ...
 /* V19:
    1) Alle Walzen starten gleichzeitig.
    2) Alle haben dieselbe Anfangsgeschwindigkeit.
    3) Die Symbole laufen sichtbar VON OBEN NACH UNTEN.
    4) Nur die Bremsphase ist pro Walze länger.
    5) Das finale 3er-Bild ist bereits Bestandteil des Strips.
       Dadurch gibt es beim Stop keinen sichtbaren Sprung mehr. */
 const runs=reels.map((reel,c)=>{
   const h=Math.max(1,reel.clientHeight);
   reel.classList.add("v19-reel-spinning");

   // Finalbild liegt ganz oben. Wir starten weiter unten im Strip und
   // bewegen den gesamten Strip nach unten bis exakt auf dieses Finalbild.
   const frames=[finalGrid[c].slice()];
   const frameCount=13 + c*2;
   for(let n=1;n<frameCount;n++) frames.push(randomTriplet());

   const strip=document.createElement("div");
   strip.className="v19-reel-strip";

   frames.forEach(group=>{
     const frame=document.createElement("div");
     frame.className="v19-reel-frame";
     frame.style.height=`${h}px`;
     group.forEach(sym=>frame.appendChild(makeSymbol(sym)));
     strip.appendChild(frame);
   });

   const travel=(frames.length-1)*h;
   strip.style.setProperty("--v19-start",`${-travel}px`);
   strip.style.setProperty("--v19-stop",`${stopTimes[c]}ms`);
   strip.style.transform=`translate3d(0,${-travel}px,0)`;

   reel.replaceChildren(strip);
   return {reel,strip,c,stop:stopTimes[c]};
 });

 // Ein gemeinsamer Layout-Punkt -> alle fünf starten im selben Frame.
 void root.offsetWidth;
 runs.forEach(({strip})=>strip.classList.add("v19-run"));

 await Promise.all(runs.map(({reel,strip,c,stop})=>new Promise(resolve=>{
   let finished=false;
   const finish=()=>{
     if(finished)return;
     finished=true;

     // Strip steht jetzt bereits exakt auf dem finalen Bild.
     // Erst nach einem Renderframe wird auf die normalen drei Zellen
     // umgestellt; optisch liegen beide Zustände pixelgleich übereinander.
     requestAnimationFrame(()=>{
       requestAnimationFrame(()=>{
         reel.replaceChildren(...finalGrid[c].map(sym=>makeSymbol(sym)));
         reel.classList.remove("v19-reel-spinning");
         resolve();
       });
     });
   };
   strip.addEventListener("animationend",finish,{once:true});
   setTimeout(finish,stop+120);
 })));
}
